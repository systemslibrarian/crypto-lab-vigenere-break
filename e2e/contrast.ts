 ...
 *    they are the surfaces its verdicts land on: `.status.ok` is
 *    `color-mix(in srgb, var(--ok) 12%, transparent)`; `.rail-step.done` and
 *    `.rail-step.active` are 10% and 12% mixes of `--ok` and `--accent`; the
 *    `<mark class="repeat-hl">` that marks every Kasiski repeat is
 *    `color-mix(in srgb, var(--accent) 42%, var(--surface-2))`; the hero aside
 *    is `color-mix(in oklab, var(--accent) 6%, transparent)`; every lit tabula
 *    recta row and column is a 14% accent mix; and the explainer's spotlight
 *    cells are a 22% accent mix. axe files all of those under `incomplete`, so a
 ...
 *    at `.85` and, far more consequentially, the explainer's alignment strip,
 *    which de-emphasises every cell OUTSIDE the spotlighted column with an
 *    inline `opacity: 0.5`. That is two thirds of the strip in the default
 *    view, carrying `--ink` and `--ink-soft` ink that does not survive the
 *    fade. axe reads the undeclared-faded colour and passes it.
 *
 ...
 *    boxes are the solver transcript (`max-height: 14rem; overflow: auto`, which
 *    overflows on every run because the transcript is one line per column plus
 *    four stage lines), the highlighted-ciphertext region built by
 *    `renderHighlightedText` (`max-height: 7rem; overflow: auto` around the full
 *    normalised ciphertext), `.tableau-wrap` (`max-height: 60vh; overflow: auto`
 *    around a 26x26 table), and every `.strip-wrap` (`overflow-x: auto` around
 *    an alignment strip wider than the card at phone width). Content scrolled
 *    past those is not dimmed or partly drawn — it is absent from the frame, and
 *    asking what colour it sits on has no answer. Its rect is still outside
 *    every ancestor's box, so the ancestor walk would find nothing behind it and
 *    fall through to white, inventing a failure. Skip it, and rely on the
 *    content that IS in view, which is measured for real.
 *
 ...
 *    draws every figure it has — the alignment strips, the IoC bar chart, the
 *    per-column frequency bars, the χ² heatmap and the tabula recta — in HTML
 *    and CSS grid rather than SVG, so the sibling walk finds nothing to do here
 *    beyond the shared header's two `aria-hidden` glyph marks. It is kept
 *    because the alternative is a helper that silently mismeasures the first
 *    labelled diagram anyone adds: an ancestor-only walk would composite a
 ...
 * that gap for this repo, and `style.css` in fact declares no `::before` or
 * `::after` of its own at all: every icon on this page (the rail's ✓ / – / … /
 * ○, the status banners' glyphs, the confidence badge's ●●○) is a real
 * `aria-hidden` text node in an element, which this walk measures directly and
 * `nontext.ts` therefore has nothing to add for.
 */
 ...
 * uses. The one genuinely short-lived rendering here is the copy buttons'
 * "Copied" swap, which `dom.ts` reverts after 1200ms; the gate scans that state
 * explicitly while it is up rather than narrowing the walk to it.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks the 26x26 tabula recta (676 `<td>`s, each owning
     * a letter), the alignment strips (one `<div class="cell">` of four `<span>`s
     * per ciphertext letter), and the column solver's 26-option `<select>` per
     * column — thousands of sibling elements all re-walking the same ancestors
     * up to `<body>`. Without the caches the pass re-reads the same computed
     ...
     * that is the solver transcript (14rem cap, `overflow: auto`), the
     * highlighted-ciphertext region (7rem cap, `overflow: auto`),
     * `.tableau-wrap` (`max-height: 60vh; overflow: auto`) and every
     * `.strip-wrap` (`overflow-x: auto`). `.sr-only` is handled separately
     * below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // its only SVG is the shared header's two `aria-hidden` marks, and the
      // menu glyph is built from three stroke-only <line>s, which is exactly
      // the shape that guard exists for. The rule is the same one that would
      // apply the moment this lab grows an SVG figure of its own.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. Here that is every
      // `<details class="aria-table">` — the text-equivalent tables under the
      // Kasiski repeats and the IoC bar chart, both of which ship shut. The gate
      // opens each by clicking its <summary>, which is the route a reader has,
      // rather than setting `.open` from script — which is exactly what the gate
      // this replaces did to all of them at once, before its single scan.
      ...
      //
      // `style.css` here DEFINES `.sr-only` with exactly that recipe
      // (`position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`) but no
      // markup currently carries the class — checked, not assumed. The guard is
      // kept rather than deleted because the class is one `el('span', {class:
      // 'sr-only'})` away from being used, and without it the first such span
      // would be reported at whatever ratio a 1px box happens to composite to,
      // for ink the compositor never lays down. It is deliberately narrow: only
      // a ZERO-AREA clip qualifies, so a partially clipped element is still
      // measured, worst case.
     ...
     *
     * What this lab hides is a closed set and every member of it is decorative:
     * the `.ico` glyph that opens each status banner (⚠ 🎯 ℹ ✓ 🔎 🔓 🛠 ⛔ 🏆 🔁
     * 👁 📖 ✗), the `.rail-ico` state marks on the progress rail (✓ – … ○), the
     * ●●● / ●●○ / ●○○ dots in `.conf-badge`, and the ⧉ on every copy button. In
     * every case the words that carry the meaning sit immediately beside the
     * glyph, in the same ink on the same surface, so measuring the sibling
     * measures the glyph too.
     *
     * Two whole graphics are hidden as well — the per-column frequency bars and
     * the 26-cell χ² heatmap — and neither owns a text node at all, so there is
     * nothing here for either oracle to have missed. Their values are reachable:
     * each column's `<select>` prints `χ²=…` per shift, and the `.hint` beneath
     * it prints the verdict and the top three candidates.
     *
     * Nothing on this page hides a VALUE. No recovered key letter, no IoC
     * number, no χ² figure, no confidence score and no verdict is inside an
     * `aria-hidden` subtree — checked rather than assumed, because that is the
     * shared blind spot where both oracles stop looking and it is the one place
     * a live readout can hide from a whole accessibility gate.
     */
     ...
     *
     * This lab has no SVG of its own — the tabula recta is a `<table>`, the
     * strips and charts are CSS grid. The guard stays because it costs nothing
     * and because the first SVG figure anyone adds would otherwise be
     * mismeasured silently: text sitting directly in a `<g>` renders nothing,
     * and a walk that measured it would report a 1:1 failure describing ink
     * that was never laid down.
     */
     ...
     *
     * It is load-bearing here rather than defensive. `style.css` sets the page
     * ground on `body` (`background: var(--bg)`) and nothing at all on `html`,
     * so the root's own paint is transparent and this is the branch that finds
     * `--bg`. The page runs several screens deep once the attack is driven — a
     * 26x26 tabula recta and five stepper sections below the fold — and `<body>`
     * has no explicit height, so the fall-through matters for every element the
     * geometry walk would otherwise leave on a white canvas the dark theme
     * never paints.
     */
