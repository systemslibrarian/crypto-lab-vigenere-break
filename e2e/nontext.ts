...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks the 26x26 tabula recta, the alignment strips
     * (four `<span>`s per ciphertext letter), and one 26-option `<select>` plus
     * a 26-cell χ² heatmap per recovered key column — thousands of sibling
     * elements all re-walking the same ancestors up to `<body>`. Without the
